...
exports.challenge = async function (p, opponent) {
    ...
    // Perbaikan: Ganti p.errorMsg dengan pengecekan fungsi yang aman atau error helper
    if (result[0][0]) {
        if (typeof p.errorMsg === 'function') {
            p.errorMsg(', There is already a pending battle!', 3000);
        } else if (p.msg && p.msg.channel) {
            p.msg.channel.createMessage(`${p.config?.emoji?.error || '❌'} **|** There is already a pending battle!`).catch(() => {});
        }
        return;
    }
    ...
    if (!player) {
        if (typeof p.errorMsg === 'function') {
            ...
            p.msg.channel.createMessage(`${p.config?.emoji?.error || '❌'} **|** You don't have a team!`).catch(() => {});
        }
        return;
    }
    ...
    if (!enemy) {
        if (typeof p.errorMsg === 'function') {
            ...
            p.msg.channel.createMessage(`${p.config?.emoji?.error || '❌'} **|** The opponent doesn't have a team!`).catch(() => {});
        }
        return;
    }
